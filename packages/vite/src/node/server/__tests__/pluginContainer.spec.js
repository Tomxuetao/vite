...
describe('plugin container', () => {
  describe('getModuleInfo', () => {
    ...
    it('can pass metadata between hooks', async () => {
      ...
      const loadResult = await container.load(entryUrl)
  ...
  describe('load', () => {
    ...
    it('can resolve a secondary module', async () => {
      ...
      await moduleGraph.ensureEntryFromUrl(entryUrl, false)
      const loadResult = await container.load(entryUrl)
      const result = await container.transform(loadResult.code, entryUrl)
...
async function getPluginContainer(inlineConfig) {
