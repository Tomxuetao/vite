...
export async function optimizeServerSsrDeps(config) {
  const ssr = true
  const cachedMetadata = await loadCachedDepOptimizationMetadata(
    config,
    ssr,
...
export function initDepsOptimizerMetadata(config, ssr, timestamp) {
...
export function addOptimizedDepInfo(metadata, type, depInfo) {
...
export function toDiscoveredDependencies(config, deps, ssr, timestamp) {
...
export function depsLogString(qualifiedIds) {
 ...
 */
export function runOptimizeDeps(resolvedConfig, depsInfo, ssr) {
...
export async function addManuallyIncludedOptimizeDeps(deps, config, ssr) {
  const { logger } = config
  const optimizeDeps = getDepOptimizationConfig(config, ssr)
  ...
  if (optimizeDepsInclude.length) {
    const unableToOptimize = (id, msg) => {
      if (optimizeDepsInclude.includes(id)) {
        logger.warn(
          `${msg}: ${colors.cyan(id)}, present in '${ssr ? 'ssr.' : ''}optimizeDeps.include'`,
        )
      }
    }
...
// Convert to { id: src }
export function depsFromOptimizedDepInfo(depsInfo) {
  const obj = {}
...
export function getOptimizedDepPath(id, config, ssr) {
...
export function getDepsCacheDir(config, ssr) {
...
function parseDepsOptimizerMetadata(jsonMetadata, depsCacheDir) {
  const { hash, lockfileHash, configHash, browserHash, optimized, chunks } =
    JSON.parse(jsonMetadata, (key, value) => {
...
export async function extractExportsData(filePath, config, ssr) {
  await init
  const optimizeDeps = getDepOptimizationConfig(config, ssr)
...
export function optimizedDepInfoFromFile(metadata, file) {
...
export async function optimizedDepNeedsInterop(metadata, file, config, ssr) {
...
export async function cleanupDepsCacheStaleDirs(config) {
  try {
