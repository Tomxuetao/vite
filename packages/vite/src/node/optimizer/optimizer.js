...
export function getDepsOptimizer(config, ssr) {
...
export async function initDevSsrDepsOptimizer(config, server) {
      ...
      if (!needsReload) {
        ...
        if (newDepsDiscovered) {
          ...
          if (needsInteropMismatch.length > 0) {
            config.logger.warn(
              `Mixed ESM and CJS detected in ${colors.yellow(needsInteropMismatch.join(', '))}, add ${needsInteropMismatch.length === 1 ? 'it' : 'them'} to optimizeDeps.needsInterop to speed up cold start`,
              {
                timestamp: true,
              },
            )
