...
export function getAffectedGlobModules(file, server) {
  const modules = []
...
export async function parseImportGlob(code, importer, root, resolveId, logger) {
  let cleanCode
  try {
...
export async function toAbsoluteGlob(glob, root, importer, resolveId) {
  let pre = ''
  if (glob[0] === '!') {
    pre = '!'
    glob = glob.slice(1)
...
export function getCommonBase(globsResolved) {
