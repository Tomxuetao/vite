...
export async function transformWithEsbuild(code, filename, options, inMap) {
  ...
  try {
    ...
    if (inMap && resolvedOptions.sourcemap) {
      ...
      map = combineSourcemaps(filename, [nextMap, inMap])
    } else {
      map =
...
export const buildEsbuildPlugin = (config) => {
  return {
...
export function resolveEsbuildTranspileOptions(config, format) {
...
let tsconfckCache
export async function loadTsconfigJsonForFile(filename) {
  try {
    if (!tsconfckCache) {
