...
export function resolveCSSOptions(options) {
...
export const isCSSRequest = (request) => CSS_LANGS_RE.test(request)
export const isModuleCSSRequest = (request) => cssModuleRE.test(request)
export const isDirectCSSRequest = (request) =>
  CSS_LANGS_RE.test(request) && directRequestRE.test(request)
export const isDirectRequest = (request) => directRequestRE.test(request)
...
export function cssAnalysisPlugin(config) {
  let server
  return {
 ...
 */
export function getEmptyChunkReplacer(pureCssChunkNames, outputFormat) {
 ...
 * @experimental
 */
export async function preprocessCSS(code, filename, config) {
...
export async function formatPostcssSourceMap(rawMap, file) {
...
const atCharsetRE =
  /@charset(?:\s*(?:"(?:[^"]|(?<=\\)")*"|'(?:[^']|(?<=\\)')*').*?|[^;]*);/g
export async function hoistAtRules(css) {
...
function loadSss(root) {
  ...
  const sssPath = loadPreprocessorPath(
    'sugarss' /* PostCssDialectLang.sss */,
    root,
  )
...
const scssProcessor = (maxWorkers) => {
  ...
  return {
    ...
    async process(source, root, options, resolvers) {
      const sassPath = loadPreprocessorPath(
...
export const convertTargets = (esbuildTarget) => {
