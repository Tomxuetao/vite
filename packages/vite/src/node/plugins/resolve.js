...
export function resolvePlugin(resolveOptions) {
  const {
    root,
    isProduction,
    asSrc,
  ...
  try {
    ...
    for (let entry of entryPoints) {
      ...
      if (resolvedEntryPoint) {
        debug?.(
          `[package entry] ${colors.cyan(idWithoutPostfix)} -> ${colors.dim(resolvedEntryPoint)}${postfix !== '' ? ` (postfix: ${postfix})` : ''}`,
        )
...
function resolveDeepImport(
  id,
  { webResolvedImports, setResolvedCache, getResolvedCache, dir, data },
  targetWeb,
  options,
) {
