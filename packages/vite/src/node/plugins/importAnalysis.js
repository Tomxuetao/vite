...
export const canSkipImportAnalysis = (id) =>
...
export function isExplicitImportRequired(url) {
    ...
    async transform(source, importer, options) {
      ...
      const normalizeUrl = async (
        ...
        if (!resolved || resolved.meta?.['vite:alias']?.noResolved) {
          ...
          return this.error(
            `Failed to resolve import "${url}" from "${normalizePath(path.relative(process.cwd(), importerFile))}". Does the file exist?`,
            pos,
          )
        }
      ...
      debug?.(
        `${timeFrom(msAtStart)} ${colors.dim(`[${importedUrls.size} imports rewritten] ${prettifyUrl(importer, root)}`)}`,
      )
      if (s) {
...
export function createParseErrorInfo(importer, source) {
